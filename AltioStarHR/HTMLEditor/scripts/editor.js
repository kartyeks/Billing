...
function set_focus() {
    ...
    try {
        ...
        var oSel = oEditor.document.body.createTextRange();

        if (this.rangeBookmark != null) {
            oSel.moveToBookmark(this.rangeBookmark);
            oSel.collapse(false);
            oSel.select();
...
function bookmark_selection() {
    var oEditor = eval("idContent" + this.oName);
    var oSel = oEditor.document.selection.createRange();
    this.rangeBookmark = oSel.getBookmark();
}
...
function InnovaEditor(oName) {
    ...
    */
    ...
    this.btnSave = false; this.btnPreview = true; this.btnPrint = false; this.btnSearch = true;
  ...
  ["grpMisc", "", ["Characters", "Line", "BRK"]]
    ...
    this.arrFontName = ["Arial Unicode MS", "Arial", "Arial Black", "Arial Narrow",
...
function doUndo() {
    ...
    if (obj.arrUndoList[0][2] == "None") {
        ...
        for (var i = 0; i < oEditor.document.all.length; i++) {
            if (oEditor.document.getElementById(i).selThis == "selThis") {
...
function RENDER(sPreloadHTML, dvId) {
    ...
    if (this.useTab && this.tabs) {
        ...
        for (var i = 0; i < this.groups.length; i++) {
            ...
            tmpTb.iconPath = this.iconPath; //this.scriptPath+this.iconPath;
...
function initISEditor() {
    ...
    //var me=this;
    //var theFrame = document.getElementById("idContent"+this.oName);
    //theFrame.onblur=function() {
    //save the selection
    //  eval(me.oName).bookmarkSelection();
    //};
...
//                break;
            ...
            case "Paragraph":
                if (oEdt.btnParagraph) {
                    tb.addDropdownButton("btnParagraph" + oName, "ddParagraph" + oName, "/btnParagraph.gif", getTxt("Paragraph"), 77);
            ...
            case "FontName":
                if (oEdt.btnFontName) {
                    //Darshan//tb.addDropdownButton("btnFontName"+oName,"ddFontName"+oName,oUtil.langDir+"/btnFontName.gif",getTxt("Font Name"),77);
                    tb.addDropdownButton("btnFontName" + oName, "ddFontName" + oName, "/btnFontName.gif", getTxt("Font Name"), 77);
                    ...
                    for (var j = 0; j < oEdt.arrFontName.length; j++) {
                        ddFont.addItem("btnFontName_" + j + oName, "<span style='font-family:" + oEdt.arrFontName[j] + "' unselectable=on>" + oEdt.arrFontName[j] + "</span><span unselectable=on style='font-family:Arial Unicode MS,tahoma'>(" + oEdt.arrFontName[j] + ")</span>");
                    }
                }
                break;
            case "FontSize":
                if (oEdt.btnFontSize) {
                    tb.addDropdownButton("btnFontSize" + oName, "ddFontSize" + oName, "/btnFontSize.gif", getTxt("Font Size"), 60);
            ...
            case "CustomTag":
                if (oEdt.btnCustomTag) {
                    tb.addDropdownButton("btnCustomTag" + oName, "ddCustomTag" + oName, "/btnCustomTag.gif", getTxt("Tags"), 60);
            ...
            case "Image":
                ...
                break;
            case "Flash":
                if (oEdt.btnFlash) tb.addButton("btnFlash" + oName, "/btnFlash.gif", getTxt("Flash"), 55);
                break;
            case "Media":
                if (oEdt.btnMedia) tb.addButton("btnMedia" + oName, "/btnMedia.gif", getTxt("Media"), 55);
                break;
            case "ContentBlock":
                if (oEdt.btnContentBlock) tb.addButton("btnContentBlock" + oName, "btnContentBlock.gif", getTxt("Content Block"));
                break;
            case "InternalLink":
                if (oEdt.btnInternalLink) tb.addButton("btnInternalLink" + oName, "/btnInternalLink.gif", getTxt("Internal Link"), 85);
                break;
            case "InternalImage":
                if (oEdt.btnInternalImage) tb.addButton("btnInternalImage" + oName, "btnInternalImage.gif", getTxt("Internal Image"));
                break;
            case "CustomObject":
                if (oEdt.btnCustomObject) tb.addButton("btnCustomObject" + oName, "/btnCustomObject.gif", getTxt("Object"), 85);
                break;
            case "Table":
                if (oEdt.btnTable) {
                    var sdd = [], sZ = 0;
...
function generateHTML() {
    ...
    "<tr><td onclick=\"" + this.oName + ".onRemoveColor();" + this.oName + ".currColor='';" + this.oName + ".hideAll()\" onmouseover=\"this.style.border='#777777 1px solid'\" onmouseout=\"this.style.border='white 1px solid'\" style=\"cursor:default;padding:1px;border:white 1px solid;font-family:Arial Unicode MS,verdana;font-size:10px;font-color:black;line-height:9px;\" align=center valign=top unselectable=on>x</td></tr>" +
    "</table></td>";
...
function refreshCustomColor() {
    ...
    sHTML = "<table cellpadding=0 cellspacing=0 width=100%><tr><td colspan=8 style=\"font-family:Arial Unicode MS,verdana;font-size:9px;font-color:black;line-height:9px;padding:1\">" + this.txtCustomColors + ":</td></tr></table>";
...
function moveTagSelector() {
    var sTagSelTop = "<table unselectable=on ondblclick='" + this.oName + ".moveTagSelector()' width='100%' cellpadding=0 cellspacing=0><tr style='background:#e9e8f2;font-family:Arial Unicode MS,arial;font-size:10px;color:black;'>" +
    ...
    //Darshan//"<img src='" + this.scriptPath + this.iconPath + "decrease.gif' style='cursor:pointer;vertical-align:middle;' onclick='" + this.oName + ".changeHeight(-50)'>" +
    //Darshan//"<img src='" + this.scriptPath + this.iconPath + "increase.gif' style='cursor:pointer;vertical-align:middle;margin-right:3px' onclick='" + this.oName + ".changeHeight(50)'>"
    "<img src='" + this.iconPath + "decrease.gif' style='cursor:pointer;vertical-align:middle;' onclick='" + this.oName + ".changeHeight(-50)'>" +
    "<img src='" + this.iconPath + "increase.gif' style='cursor:pointer;vertical-align:middle;margin-right:3px' onclick='" + this.oName + ".changeHeight(50)'>"
    "</td></tr></table>";

    var sTagSelBottom = "<table unselectable=on ondblclick='" + this.oName + ".moveTagSelector()' width='100%' cellpadding=0 cellspacing=0><tr style='background:#e4e3ed;font-family:Arial Unicode MS,arial;font-size:10px;color:black;'>" +
    "<td id=idElNavigate" + this.oName + " style='padding:1px;width:100%' valign=top>&nbsp;</td>" +
    "<td align=right valign='center' nowrap>" +
    "<span id=idElCommand" + this.oName + " unselectable=on style='vertical-align:middle;display:none;text-decoration:underline;cursor:hand;padding-right:5;' onclick='" + this.oName + ".removeTag()'>" + getTxt("Remove Tag") + "</span>" +
    "<img src='" + this.iconPath + "decrease.gif' style='cursor:pointer;vertical-align:middle' onclick='" + this.oName + ".changeHeight(-50)'>" +
...
function runtimeBorder(bToggle) {
    ...
    else {//refresh based on the current status
...
function cleanTags(elements, sVal)//WARNING: Dgn asumsi underline & linethrough tidak bertumpuk
{
    ...
    while (elements.length > 0) {
        f = elements[0];
        if (f.hasChildNodes && f.childNodes.length == 1 && f.childNodes[0].nodeType == 1 && f.childNodes[0].nodeName == "SPAN") {//if font containts only span child node
            ...
            if (f.parentElement.nodeName == "SPAN" && f.parentElement.childNodes.length == 1) {//font is the only child node of span.
...
function applyFontSize(val) {
    ...
    if (oSel.parentElement) {
        ...
        for (var i = 0; i < allSpans.length; i++) {
            tempRange.moveToElementText(allSpans[i]);
            if (oSel.inRange(tempRange)) {//tdk perlu ada +/- (krn optionnya tdk ada)
...
function copyStyleClass(newSpan, arrStyles, sClassName) {
    ...
    if (newSpan.style.fontFamily == "") {//Text Formatting Related
...
function replaceWithSpan(oEditor, arrStyles, sClassName) {
    ...
    else {//selection over paragraphs
    ...
    if (oSpanStart) {//WARNING: Jika tdk ada span, kemungkinan selection failed?
...
function doKeyPress(evt, oName) {
    ...
    if (evt.ctrlKey) {
        if (evt.keyCode == 89) {//CTRL-Y (Redo)
            if (!evt.altKey) eval(oName).doRedo();
        }
        if (evt.keyCode == 90) {//CTRL-Z (Undo)
            if (!evt.altKey) eval(oName).doUndo();
        }
        if (evt.keyCode == 65) {//CTRL-A (Select All) => spy jalan di modal dialog
            if (!evt.altKey) eval(oName).doCmd("SelectAll");
        }
        if (evt.keyCode == 66 || evt.keyCode == 73 || evt.keyCode == 85) {//CTRL-B (Bold, Italic, Underlined)
...
function recur(oEl, sTab) {
    ...
    for (var i = 0; i < oEl.childNodes.length; i++) {
        ...
        if (oNode.outerHTML) {
            if (oNode.outerHTML.substring(0, 2) == "<" + "%") {//server side script
