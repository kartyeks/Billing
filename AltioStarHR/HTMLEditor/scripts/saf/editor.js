/***********************************************************
InnovaStudio WYSIWYG Editor 4.0
  ...
  if(this.mode=="XHTML"){this.btnXHTMLFullSource=true;this.btnXHTMLSource=false;}
    ...
    if(this.useTab && this.tabs) {
      ...
      for (var i=0; i<this.groups.length;i++) {
        ...
        tmpTb.iconPath = this.scriptPath.substring(0, this.scriptPath.indexOf("scripts/saf/")) + this.iconPath;
...
function loadHTML(sHTML)//hanya utk first load.
    ...
    if(this.publishingPath!="")
        ...
        if(!arrA)
            {//if no <base> found
            sHTML=this.docType+"<html><head><base href=\""+this.publishingPath+"\"/>"+defaultEditCss+this.headContent+"</head><body>" + sHTML + "</body></html>";
            //kalau cuma tambah <HTML><HEAD></HEAD><BODY.. tdk apa2.
            }
        }
    else
        {
        sHTML=this.docType+"<html><head>"+defaultEditCss+this.headContent+"</head><body>"+sHTML+"</body></html>";
        }
    oDoc.write(sHTML);
    oDoc.close();
...
function loadHTMLFull(sHTML, firstLoad)//first load full HTML
  ...
  /*** end ***/
    ...
    //var defaultEditCss="<link id='innova' href='"+this.scriptPath+"default_edit.css' type='text/css' rel='stylesheet' />";
    //if(sHTML.match(/<head>/gi)) {
    //  sHTML = sHTML.replace(/<head>/gi, "<head>"+defaultEditCss); 
    //} else {
    //  sHTML = sHTML.replace(/<body/gi, "<head>"+defaultEditCss+"</head><body"); 
    //}    
...
function moveTagSelector()
    ...
    var icPath=this.scriptPath.substring(0, this.scriptPath.indexOf("scripts/saf/"))+this.iconPath;    
    /*
