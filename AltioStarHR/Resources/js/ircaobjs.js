...
function IRCAMAINOBJ(){ // constructor
	this.id="IRCA MAIN";this.window=window;this.document=this.window.document;this.readOnly=false;this.unLockPg="";this.ReqXMLstr="";
...
IRCAMAINOBJ.prototype.getNewInstance=function (windref){return new this.window.IRCAOBJ(windref,this);}
...
IRCAMAINOBJ.prototype.getValue=function (msgID,langID,fileName){
    if (!window.MsgXmlDoc){
        try{
            if(window.ActiveXObject){window.MsgXmlDoc = new ActiveXObject("MSXML2.DOMDocument");}
            else{window.MsgXmlDoc = document.implementation.createDocument;}
...
IRCAMAINOBJ.prototype.showWindow=function(type,path,name,windarg,windfeatures,parentwind,withObj,xarg){
    ...
    //guru lock
    this.window.drawDialog(path,true,false,'RCAT Platinum - Selection',parentwind,windfeatures.dialogHeight,windfeatures.dialogWidth);
    break;
    }
};
IRCAMAINOBJ.prototype.regWindow=function(name,windowref,pname,isNewWind){
    ...
    WinReg.push(name);
//    if(!isNewWind){
//	    windowref.execScript('window.name="'+name + '";window.pname="' + pname + '";window.opener=window.dialogArguments[0]');
//    }
};
...
IRCAMAINOBJ.prototype.WriteMeUp=function(pwind,mewind,pgstr){
	...
	if(pgstr){
		...
		mewind.execScript("window.ircaObject=window.dialogArguments[0].ircaObject.ircaMain().getNewInstance(window)");
		newdoc.write(pgstr);
		newdoc.namespaces.add("IRCAAuth",pwind.document.namespaces("IRCAAuth").urn);
		mewind.execScript('window.attachEvent("onunload",function(){ircaObject.ircaMain().unRegWindow(window.name,window.dialogArguments[0].name,window.boolprefresh||false)})');
		newdoc.close();
	};
};
...
IRCAMAINOBJ.prototype.makeBW=function(docObj){
   if(!docObj){docObj=window.document}; if(!docObj.body){return};
...
IRCAMAINOBJ.prototype.lockControl=function(controlObj){
    ...
    controlObj.attachEvent('onresize',function(){this.window.IRCAMAIN.lockWindowOnResize(null,controlObj);});
};
IRCAMAINOBJ.prototype.unLockControl=function(controlObj){
        ...
        if(irLocker){
            controlObj.detachEvent('onresize',function(){this.window.IRCAMAIN.lockWindowOnResize(null,controlObj);});
            irLocker.removeNode();
        };
};
...
function getUniqueUrl(strUrl){
   var dtsend=this.window.IRCAMAIN.getUniqueID();
		if (strUrl.indexOf("?")>0)
...
function getFile(url,boolasync,returnContent,funcref,NoUniqueUrl){
	...
	if(typeof(funcref)!='undefined' && boolasync)
	    NewExeObjx.onreadystatechange=function(){ 
            ...
            }else if (NewExeObjx.readyState == 4 && NewExeObjx.status==403){this.window.IRCAMAIN.absend();return}
        }
...
function isClientOffLine(){
    try{
        return false; //return !window.navigator.onLine;       
...
IRCAMAINOBJ.prototype.drawDialog=function (pageSrcA,isURL,isScrollable,title,dialogArgs,dialogHeight,dialogWidth,dialogTopP,dialogLeftP,resizable){
    ...
    if(window.WPDiv1_ifrm && window.WPDiv1_ifrm.document){
        if(isURL){
...
IRCAMAINOBJ.prototype.private_initDialog=function (argumentsA){
    ...
    if(window.ChildWB)WPDiv1_ifrm.opener=ChildWB;//need to change in future
    WPDiv1_ifrm.selfID=RcatWind.id;
    WPDiv1_ifrm.close=function(){ IRCAMAIN.window.closeDialog(WPDiv1_ifrm.selfID); };
...
IRCAMAINOBJ.prototype.write2Dialog=function (ContentX,isRewrite){
    ...
    WPDiv1_ifrm.execScript('window.ircaObject=window.parent.IRCAMAIN.getNewInstance(window)');
...
IRCAMAINOBJ.prototype.absend =function (forstr){//abnormal session end
    __Alert("Sorry! Your Session was ended by the Server. \n\n Please Login again to continue.");this.AppDispose('window.open(".?ru");');return;
};
IRCAMAINOBJ.prototype.AppDispose=function(strRunBeforeClose){
	...
	this.window.CleanExit=true;
	this.unRegWindows();
	try{
	if(strRunBeforeClose){ this.window.execScript(strRunBeforeClose);} }catch(e){}
	this.window.close();
};
IRCAMAINOBJ.prototype.AppLock=function(boolUnLock,id,zindx){
    if(boolUnLock){
        //if(this.window.dhtmlwindow && this.window.dhtmlwindow.getLength()<2)
        this.windUnLock(id);
    }else{
...
try{
	...
	if(!this.domObj && this.domObj.selectNodes(this.root + path).length<1){if(!retobj){return ""}else{return null};};
		if(!prop){
			if(!retobj){
				//return eval("window.IRCAMAIN.domXml.domObj.selectNodes('"+this.root + path+"').text" );
				return this.domObj.selectNodes(this.root + path).text;
			}else{
				//return eval("window.IRCAMAIN.domXml.domObj.selectNodes('"+this.root + path+"')" );
				return this.domObj.selectNodes(this.root + path);
			}
		}else{
			//return eval("window.IRCAMAIN.domXml.domObj.selectNodes('"+this.root + path+"')." + prop );
...
IRCAMAINOBJ.prototype.WrapRequest = function (CntrolId,EvntType,PDOC,boolasync,pgtype,callbackfunc,paramx,reqWind){
	...
	this.AppLock();this.window.ircaObject.setObjectProp('pb2','style','display','');
    if(boolasync==null){boolasync=true};
	...
	if (!window.xmlreqtmpl)xmlreqtmpl= new ActiveXObject("MSXML2.DomDocument");
	if (xmlreqtmpl.xml==""){
	...
	xmldom.selectSingleNode("//Page/@CurMode").text =this.window.CurMode||"";
	xmldom.selectSingleNode("//Page/@TargetMode").text =this.window.TargetMode||"";
	...
	if(this.window.ircaObject.getReadLock() && !boolInciFlashRep){CanSend=false;};
...
if (!window.XmlDC)XmlDC = new ActiveXObject("MSXML2.DomDocument");	
...
IRCAMAINOBJ.prototype.WrapResponse=function (xmlobj,PDOC,reqWind,async){
    if(async)
        window.setTimeout(new WrapResponseX(this,xmlobj,PDOC,reqWind),100);
    else
        ...
        try {
            ...
            switch (pgStatus) {
                case "0": case "OK":
                    ...
                    if (tmptPagetype == "next" || tmptPagetype == "popup") { thisObj.window.FNWrapResponse(xmlobj, PDOC, reqWind); return };
            ...
            thisObj.window.ircaObject.setObjectProp('pb2', 'style', 'display', 'none');
                    ...
                    if (cntrlname != "") {
                        switch (itmprop) {
                            case "disabled": case "title":
                                //				            itmexec="PDOC.getElementById('"+ cntrlname +"')."+itmprop +" = " + p1 + thisObj.window.ircaObject.SplitStrB
                                itmexec = p1 + thisObj.window.ircaObject.SplitStrB;
                            ...
                            case "":
                                break;
                            default:
                                //				            itmexec="PDOC.getElementById('"+ cntrlname +"').style."+itmprop +" = " + p1 + thisObj.window.ircaObject.SplitStrB
                                itmexec = p1 + thisObj.window.ircaObject.SplitStrB;
                                //				            PDOC.parentWindow.execScript(itmexec,"javascript");  
                                var g = PDOC.getElementById(cntrlname);
            ...
            switch (cntrltype) {
                ...
                case "hidden": case "text": default:
                    try {
                        ...
                        PDOC.parentWindow.execScript('document.getElementById(\'' + cntrlname + '\').' + xName + '= window.tempX5487Value');
...
IRCAMAINOBJ.prototype.ThisPage= function (thisdoc){
	if (!thisdoc){thisdoc=this.window.document}
	var strpg="";
...
function IRCAOBJ(windref,IRCAMAINOBJref,IsMain){ // constructor
	this.window=windref||window;this.document=this.window.document;this.IRCAMAIN=IRCAMAINOBJref;
	this.page='';this.id="IRCAGEN_" + this.page;this.name=this.window.name;try{this.pname=this.window.opener.name||this.window.pname}catch(e){};this.pname+='';
	this.query=this.window.qrystr||'';this.isWindowRefresh=false;this.dtformat="";this.PageInitB=false;this.pageTitle='';
	this.DelString='';this.isPageCanceled=false;this.enablePartialLoadCaching=true;this.plCache=[];
	this.Window_Init(IsMain);this.dhtmlwindow=this.window.dhtmlwindow; 
};
IRCAOBJ.prototype.Window_Init=function(IsMain){
	if(this.ircaMain()){this.ircaMain().unLockPg=""};
	if(!this.window.document.oncontextmenu){this.window.document.oncontextmenu=this.DocContext};this.window.document.onselectstart=this.DocContext;if (this.window.document.onkeydown==null){this.window.document.onkeydown=this.DocKD};
	this.window.docready=function(){var Objirca = this.window.ircaObject; Objirca.Page_State(Objirca);}	
	this.Page_InitSplForNetAdv(IsMain);
};
IRCAOBJ.prototype.Page_InitSplForNetAdv=function(IsMain){
	if(!IsMain){
	    if(!this.window.igtbl_initGrid){
	        this.window.initGrid=[];this.window.igtbl_initGrid=function(gridId){this.window.initGrid[this.window.initGrid.length]=gridId;this.ircaObject.includeGridScripts();}
	    }
	    if(!this.window.igtree_initTree){
	        this.window.initTree=[];this.window.igtree_initTree=function(treeId){this.window.initTree[this.window.initTree.length]=treeId;this.ircaObject.includeTreeScripts();}
	    }
	    if(!this.window.ig_CreateWebScheduleInfo){
	        this.window.initWebScheduleFuncs=[];
	        var oWebScheduleInfo1Obj={window:this.window,updateControlState:function(){this.window.initWebScheduleFuncs[this.window.initWebScheduleFuncs.length]=['oWebScheduleInfo1.updateControlState',arguments];}};
	        this.window.ig_CreateWebScheduleInfo=function(){this.window.initWebScheduleFuncs[this.window.initWebScheduleFuncs.length]=['ig_CreateWebScheduleInfo',arguments];return oWebScheduleInfo1Obj;}
	        this.window.ig_CreateCalendar=function(){this.window.initWebScheduleFuncs[this.window.initWebScheduleFuncs.length]=['ig_CreateCalendar',arguments];}
	        this.window.ig_CreateWebDialogActivator=function(){this.window.initWebScheduleFuncs[this.window.initWebScheduleFuncs.length]=['ig_CreateWebDialogActivator',arguments];}
	        this.window.igcal_init=function(){this.window.initWebScheduleFuncs[this.window.initWebScheduleFuncs.length]=['igcal_init',arguments];}
	        this.window.ig_CreateWebMonthView=function(){this.window.initWebScheduleFuncs[this.window.initWebScheduleFuncs.length]=['ig_CreateWebMonthView',arguments];}
	    }
	}
	this.window.igtbl_tableMouseMove=this.dumpF;this.window.igtbl_headerMouseOut =this.dumpF;this.window.igtbl_headerMouseMove =this.dumpF;this.window.igtbl_headerMouseOver =this.dumpF;this.window.igtbl_cellMouseMove =this.dumpF;this.window.igtbl_cellMouseOver =this.dumpF;
	this.window.igtbl_cellMouseOut =this.dumpF;this.window.igtree_mouseover =this.dumpF;this.window.igtree_mouseout =this.dumpF;this.window.igtree_onscroll =this.dumpF; if(!this.window.igtbl_activate)this.window.igtbl_activate =this.dumpF;
};
IRCAOBJ.prototype.Page_Init=function(){
	this.PageInitB=true;
	this.page=this.ThisPage();this.id="IRCAGEN_" + this.page;
	//this.setFormTitle();
//	if(this.window.name=='ChildWB'){
//		if(!this.ircaMain().readOnly && this.ircaMain().unLockPg!=this.id){this.ircaMain().setFormLock(false)}
//	}
	this.window.__doPostBack=this.__overridedoPostBack;
	if(!this.window.allowSumbit){
	    this.window.document.forms[0].attachEvent("onsubmit",function(){ return this.window.allowSumbit == true});
	}
	//if(this.window.initGrid.length>=1){this.Grid_Init();}
	//if(this.window.initTree.length>=1){this.Tree_Init();}
	if(this.window.initWebScheduleFuncs && this.window.initWebScheduleFuncs.length>0){this.WebSchedule_Init();}
	if(!this.dhtmlwindow){this.dhtmlwindow=this.ircaMain().window.dhtmlwindow};
	//if(this.window.InitializeComponents){this.window.InitializeComponents(null,true)}
	if (!this.window.disableDataCall) this.window.setTimeout(new initDataCall(this, null, true, true), 1);
    return;
};
...
IRCAOBJ.prototype.startDataCall=function(QueryString,blnasync){
    var thisObj=this.startDataCall?this:this.window.ircaObject();
...
function callInitializeComponents(thisObj,canCallInitFunc){
    ...
    if(thisObj.window.InitializeComponents){thisObj.window.InitializeComponents(null,true)};
}
...
IRCAOBJ.prototype.Grid_Init=function(strGrid,reload){
	if(!this.window.initGrid){return;};
	for(var j=0;j<this.window.initGrid.length;j++){
		strGrid=this.window.initGrid[j];
		if(!strGrid){break};
		this.window.execScript('this.window.o' + strGrid +'= this.window.igtbl_initGrid("' + strGrid + '")');
		var gridDiv= this.getObject(strGrid + '_div');if(gridDiv){gridDiv.setAttribute("IRCAType","grid");
		this.window.eval('this.window.o' + strGrid + '.NeedPostBack = false;');}
	}
	this.window.initGrid=null;
};
IRCAOBJ.prototype.WebSchedule_Init=function(){
	if(!this.window.initWebScheduleFuncs){return;};
	var Func2Call;var FuncArguments;
	for(var j=0;j<this.window.initWebScheduleFuncs.length;j++){
	    Func2Call=this.window.initWebScheduleFuncs[j][0];
		FuncArguments=this.window.initWebScheduleFuncs[j][1];
		if(!Func2Call){break};
		//diff
		//try{
		    var Func=this.window.eval('window.'+Func2Call);
		    if(!Func){break};
		    if(Func2Call=='ig_CreateWebScheduleInfo'){
		        this.window.oWebScheduleInfo1=Func.apply(this,FuncArguments);
		    }else if(Func2Call=='oWebScheduleInfo1.updateControlState'){
		        Func.apply(this.window.oWebScheduleInfo1,FuncArguments);
		    }else{
		        Func.apply(this,FuncArguments);
		    }
		//}catch(e){;};
	}
	this.window.initWebScheduleFuncs=null;
};
IRCAOBJ.prototype.Tree_Init=function(strTree){
	if(!this.window.initTree){return;};
	for(var j=0;j<this.window.initTree.length;j++){
		strTree=this.window.initTree[j];
		if(!strTree){break};
		this.window.eval('this.window.o' + strTree +'= this.window.igtree_initTree("' + strTree + '")');
		this.getObject('T_'+strTree).setAttribute("IRCAType","tree");
	}
	this.window.initTree=null;
};
...
IRCAOBJ.prototype.XPInfo=function(ObjId,infoText,title){
    if(!this.window.XPAlert || !ObjId){return};if(!title){title=this.pageTitle};var Obj=this.getObject(ObjId);
    this.window.XPAlert(Obj,'<b>'+title+'</b>',infoText,10,'Info')
};
IRCAOBJ.prototype.ShowParentPopup=function (Url,title,type,isModal,windw){
    if(typeof(Url)=='undefined'||Url==null)return;
    if(!this.dhtmlwindow)return;
    if(typeof(type)=='undefined'||type==null)type='iframe';if(!windw){windw=this.window;/*if(!windw.isPopup)windw=this.ircaMain().window.ChildWB*/};
...
IRCAOBJ.prototype.ShowPopupDiv=function (divID,title,prop,isModal,windw){
    ...
    //if(!this.window.document.getElementById(divID))return null;
...
IRCAOBJ.prototype.ircaMain=function(){return this.IRCAMAIN||this.window.IRCAMAIN};
...
IRCAOBJ.prototype.isNothing=function(objchk){return this.window.document.getElementById(objchk)==null?true:false;}
...
IRCAOBJ.prototype.getObject=function(objchk){return this.window.document.getElementById(objchk);}
IRCAOBJ.prototype.getObjectValue=function(obj){return this.window.document.getElementById(obj)==null?"":this.getObject(obj).value.trim();}
IRCAOBJ.prototype.getObjectProp=function(obj,propstr){if(!this.getObject(obj)){return ""};return this.window.eval("document.getElementById('"+ obj + "')." + propstr);}
IRCAOBJ.prototype.setObjectValue=function(obj,xvalue,typ){if(!typ){typ="0"};var shtml=false;var typ=typ.split("/");if(typ.length>1 && typ[1].toLowerCase()=="h"){shtml=true};if(!this.getObject(obj)|| xvalue==null){return};xvalue=this.escapeChars(xvalue);var tmpobj=this.getObject(obj);switch (parseInt(typ[0],10)){case 0 :tmpobj.value=xvalue;break;case 1:if(shtml){tmpobj.innerHTML=xvalue}else{tmpobj.innerText=xvalue};break;case 2:var sval="";var stext="";if(typeof(xvalue=="string")){sval=xvalue;stext=sval}else{sval=xvalue[0][0];stext=xvalue[0][1]}Add2Lstbx(tmpobj,stext,sval,this.window.document);break;case 3:tmpobj.checked=xvalue;break;}}
...
IRCAOBJ.prototype.pause=function(timeout) { var dialogScript = 'window.setTimeout(' + ' function () { window.close(); }, ' + timeout + ');'; var disablwin = this.window.showModalDialog('javascript:this.window.document.writeln(' + '"<script>' + dialogScript + '<' + '/script>")'); }
IRCAOBJ.prototype.queryString=function(key){ var page = new PageQuery(this.window.location.search);return unescape(page.getValue(key)); } 
IRCAOBJ.prototype.toolTipHelp=function(vartxt){var oPopup = this.window.createPopup();var oPopBody = oPopup.document.body;oPopBody.style.backgroundColor = "lightyellow";	oPopBody.style.border = "solid black 1px";oPopBody.innerHTML=vartxt;oPopup.show(window.event.clientX, window.event.clientY, 180, 125, document.body);}
...
IRCAOBJ.prototype.startPrintData=function(dataid,title){
    ...
    if(!isTextHTML(dataid)){    
        var dataObject=this.window.document.all[dataid];
    ...
    try{
    var lnkmediaprint = this.removeMediaPrint(this.window.document);
    var temptag='<IFRAME id="winprintData"  style="width: 1px; height: 1px;" src="APPRES.ashx/1664805296/_blank.htm"></IFRAME>'
    this.window.document.body.insertAdjacentHTML('beforeEnd',temptag);
    var docC=this.window.winprintData.document.open('text/html', 'replace');
    docC.write(data);docC.title=title+'';docC.close();
    this.window.winprintData.focus();
    this.window.winprintData.print();
    this.window.winprintData.frameElement.removeNode();
    }catch(e){
...
IRCAOBJ.prototype.endPrintData=function(){
    var elewinprintData=this.window.document.getElementById('winprintData');
    if(elewinprintData){
...
IRCAOBJ.prototype.WrapRequest=function(CntrolId,EvntType,PDOC,boolasync,pgtype,callbackfunc,paramx,reqWind){if(!PDOC){PDOC=this.window.document};if(!reqWind){reqWind=this.window};return this.ircaMain().WrapRequest(CntrolId,EvntType,PDOC,boolasync,pgtype,callbackfunc,paramx,reqWind);};
...
IRCAOBJ.prototype.confirmDeleteNode=function (curnode){
	...
	var retv=this.window.confirm("Do you really want to Delete?");
	return retv;
};
...
IRCAOBJ.prototype.getReadLock=function(){try{return (this.ircaMain().readOnly && (this.ircaMain().window.CurMode!="search" && this.ircaMain().window.CurMode!="setup" && this.ircaMain().window.CurMode!="report"));}catch(e){return false}}
...
IRCAOBJ.prototype.fixScriptinHTML=function(thisObjRef,tmpstr){
   ...
   var onerrorfuncRef=thisObjRef.window.onerror;
   thisObjRef.window.onerror=function(){return true};
    try{
        ...
        while( (curpos<siz)||s!=-1||e!=-1){
            ...
            exc=exc.replace('// -->','');
            exc=exc.replace('-->','');
            try{ thisObjRef.window.execScript(exc); }catch(r){;};
        }
    }catch(s){;};
    thisObjRef.window.onerror=onerrorfuncRef;        
};	  
IRCAOBJ.prototype.partialLoad=function(eventTarget, eventArgument,containerObj,useDirectResponse){
    ...
    if(!containerObj){return}; 
  	...
  	var tmpstr;var fromCache=false;var uniqueID=this.window.ircaObject.getHash(eventTarget+eventArgument);
  	//if caching is enabled then check if its available
  	if(this.window.ircaObject.enablePartialLoadCaching){
 	  tmpstr=this.window.ircaObject.plCache[uniqueID];fromCache=true;
  	};
  	if(!tmpstr){
  	    fromCache=false;
	    var thispageQry=(this.window.document.body.ThisPagePath || '') + this.window.ircaObject.ThisPage();
	    var theFormvalues = '';	var theform = this.window.document.forms[0];
	   ...
	   // if (!this.window.qrystr && this.window.document && this.window.document.forms[0]){
	        this.window.qrystr=unescape(this.window.document.forms[0].action.split('?')[1]);
	   // }
	    if(this.window.qrystr){
		    if(thispageQry.indexOf("?")>=0){
			    thispageQry +=  '&' + this.window.qrystr;
		    }else{
			    thispageQry += '?'+this.window.qrystr;
		    }
	    };
 	    this.window.ircaObject.ircaMain().AppLock();
        tmpstr=this.window.ircaObject.ircaMain().ExecutePFunc(thispageQry,theFormvalues,false,true);
 	    this.window.ircaObject.ircaMain().AppLock(true);
    }
    ...
    var tmpNetAdvGridInitFunc=this.window.igtbl_initGrid;
    //nullify the actual function, since Page_InitSplForNetAdv will not replace if it is already ther.
    if(hasigtblGrid)this.window.igtbl_initGrid=null;
	//call the replacement method 
	if(hasigtblGrid)this.window.ircaObject.Page_InitSplForNetAdv();
    containerObj.innerHTML='';
    containerObj.insertAdjacentHTML('afterBegin',tmpstr);
    //for script execution
    this.window.ircaObject.fixScriptinHTML(this,tmpstr);
	//replace the method with original method
    if(hasigtblGrid)this.window.igtbl_initGrid=tmpNetAdvGridInitFunc;	
	//now actually call the original method
    if(hasigtblGrid) this.window.ircaObject.Grid_Init('',true);
    if(!fromCache && this.window.ircaObject.enablePartialLoadCaching){
        if(!this.window.ircaObject.plCache){
            this.window.ircaObject.plCache=[];
        }
         this.window.ircaObject.plCache[uniqueID]=tmpstr;
    }
};
IRCAOBJ.prototype.__overridedoFormSubmit=function (eventTarget, eventArgument,xFormValues) {
    if(!this.window.allowSumbit){return ''};
	if(this.window.beforePageChange){this.window.beforePageChange()};	
	// note how form.onsubmit is being called explicity 
	var thispageQry=(this.window.document.body.ThisPagePath || '') + this.window.ircaObject.page;
	var theFormvalues = '';	var theform = this.window.document.forms[0];
	...
	if(xFormValues){//sort code
	    ...
	    theFormvalues += xFormValues;this.window.xFormValues=xFormValues;
	}else if(this.window.xFormValues){
	...
	this.window.ircaObject.ircaMain().AppLock();
    var tmpstr=this.window.ircaObject.ircaMain().ExecutePFunc(thispageQry,theFormvalues,false,true);
	this.window.ircaObject.ircaMain().AppLock(true);
	if (!tmpstr){return ''}
	CollectGarbage();this.window.__doPostBack=this.__overridedoPostBack;
	return tmpstr;
};
IRCAOBJ.prototype.__normaldoPostBack=function(eventTarget,eventArgument){
    if(!this.window.allowSumbit){return ''};
    var theForm = this.window.document.forms[0];
    if (!theForm) {
...
IRCAOBJ.prototype.__overridedoPostBack=function (eventTarget, eventArgument,xFormValues) {
	if(!eventArgument){if(this.window.allowSumbit){this.window.ircaObject.__normaldoPostBack(eventTarget, eventArgument) }; return}; 
	if(!eventArgument.contains('SORT') && !eventArgument.contains('Page')){return}
	//Fire event if the function exists
	if(this.window.beforePageChange){this.window.beforePageChange()};	
	// note how form.onsubmit is being called explicity 
	var thispageQry=(this.window.document.body.ThisPagePath || '') + this.window.ircaObject.page;
	if(this.window.igtbl_formSubmit){this.window.igtbl_formSubmit(this.window.eval('o'+eventTarget))};
	var theFormvalues = '';	var theform = this.window.document.forms[0];
	theFormvalues  = '__EVENTTARGET=' + escape(eventTarget.split("$").join(":")) + '&';
	theFormvalues += '__EVENTARGUMENT=' + escape(eventArgument) + '&';
	...
	if(this.window.qrystr){
		...
		if(theform['ptype']){
			thispageQry += '?gridtype=' + escape(this.window.ircaObject.getObjectValue('ptype'));
		}
	}
	//bug fix for paging misplaced in page
	var GridParentElement=null;
	var tmpGridMainObj=this.window.ircaObject.getObject(eventTarget + '_main');
	if(tmpGridMainObj && tmpGridMainObj.parentNode.id!=this.window.document.forms[0].id){
	...
	if(eval("typeof(o"+eventTarget+")=='object'")){
		this.window.igtbl_unloadGrid(eventTarget);
	}
	this.window.ircaObject.setObjectProp(eventTarget,'outerHTML',null,'');
	this.window.ircaObject.setObjectProp(eventTarget + '_main','outerHTML',null,'');
	CollectGarbage();
    if(GridParentElement){
        GridParentElement.insertAdjacentHTML('afterBegin',tmpstr);
    }else{
	    this.window.document.forms[0].insertAdjacentHTML('beforeEnd',tmpstr);//
    }
    //for script execution
    this.window.ircaObject.fixScriptinHTML(this,tmpstr);
	this.window.ircaObject.fireObjectMethod(eventTarget + '_div','setAttribute','"IRCAType","grid"');
    //bug fix for infragistics - currentPageIndex not updated after page navigation
    this.window.ircaObject.window.eval('o'+ eventTarget + '.CurrentPageIndex='+ parseInt(eventArgument.replace('Page:',''))||1);
    if(this.window.ircaObject.window.sortConfig && this.window.ircaObject.window.sortConfig[eventTarget]){
        this.window.ircaObject.window.sortConfig[eventTarget]['page']=parseInt(eventArgument.replace('Page:',''))||1;
    }
	if(this.window.afterPageChange){this.window.afterPageChange()};	
	this.window.__doPostBack=this.window.ircaObject.__overridedoPostBack;
};
