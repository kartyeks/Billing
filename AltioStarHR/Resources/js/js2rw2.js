...
var isIEX = (navigator.appName == "Microsoft Internet Explorer");
var string = '';

if (window.onerror)
    window.onerror = function(ev) { HandlePageError(ev) };

var __Alert = alert;

if (window.onHelp)
    window.onHelp = function(ev) { ShowHelp(ev) };

var open = OpenWindow;

var thisOpenWin = open;
...
function isNull(checkObject) {
    return ((checkObject == null) || ((checkObject) == undefined));
};

function replace(strExp, strFnd, strRpl) {
    return strExp.__replaceAll(strFnd, strRpl);
}

function indexOfarray(varray, strId) {
    var i;
    for (i = 0; i < varray.length; i++) {
        if (varray[i] == strId) { return i; };
    }
}
...
function _window_onload() {
    document.detachEvent('onreadystatechange', _window_onload);
    if (document.readyState == "complete") {
        if (ircaObject && ircaObject.docready) { ircaObject.docready(); }
    } else {
        if (isIEX) {
            document.attachEvent('onreadystatechange', _window_onload);
        } else {
            addEventListener('onreadystatechange', _window_onload, false);
        }
    };
}
...
if (isIEX) {
    attachEvent('onload', _window_onload);
} else {
    addEventListener('onload', _window_onload, false);
}
...
function ShowHelp(ev) {
    ...
    var curHelpTopicID = ''; try { curHelpTopicID = document.body.HelpTopicID; } catch (e) { ; };
    if (string.isEmpty(curHelpTopicID)) { curHelpTopicID = '' } else { curHelpTopicID = 'IRCAHLPTID' + curHelpTopicID };
    try {
        open("IS91HelpProvider.axd/" + curHelpTopicID, 'IRCAWebHelp', "fullscreen=0,height=400,width=700,left=150,top=150,directories=0,location=0,menubar=0,resizable=1,scrollbars=1,status=0,titlebar=0,toolbar=0");
    } catch (e) {
        showModelessDialog("IS91HelpProvider.axd/" + curHelpTopicID, window, 'status:0;help:0;resizable:1;');
    };
    return false;
}
...
function removeRow(grdId, rwNo) {
    if (grdId) {
        var oGrid = igtbl_getGridById(grdId);
        if (oGrid) {
            if (rwNo < 0) return;
            oGrid.Rows.remove(rwNo);
            if (oGrid.Rows.length > 0) oGrid.setActiveRow(oGrid.Rows.getRow(0).Id);
        }
        if (ircaObject) { ircaObject.setObjectSrc('img_uwgDoc', 'APPRES.ashx/1664805296/nothumb.gif') };
    }
}
...
function dialogAutoResize() {
    if (resizeDialog && document.Form1) {
...
function uwGrid_BeforeSortColumnHandler(gridName, columnId, pageNumber, sortordr) {
    ...
    if (!sortConfig) { sortConfig = [] };
    var sortConfigGrid = sortConfig[gridName];
    if (!sortConfigGrid) {
        sortConfig[gridName] = []; sortConfigGrid = sortConfig[gridName];
...
calendar = null;
function DTPICKERshow(ev) {

    if (!ev) var ev = window.event;

    attachedctl = ev.srcElement ? ev.srcElement.id : ev.target.id;
...
function OnlyNumbers(ev) {
    ...
    var charact = (ev.keyCode) ? ev.keyCode : ((ev.charCode) ? ev.charCode : ev.which);
    ...
    if (charact < 48 || charact > 57) {
        ...
        (ev.keyCode) ? ev.keyCode = 0 : charact = 0;
        return false;
    }
...
function OnlyNumbersWithDec(val, ev) {

    var charact = (ev.keyCode) ? ev.keyCode : ((ev.charCode) ? ev.charCode : ev.which);

    if (charact == 46 && val.indexOf(".") < 0) return false;
...
function UnlockWin(PDOC, reqWind, tmptPagetype) {
    ...
    onmousedown = LWhandleM; onmousedown = LWhandleK;
}
...
function setSuggestValue(rw) {
    ...
    //ircaObject.setObjectValue(controlName+"___userinput",ContainsAmp(rw.getCell(1).getValue()));
    //ircaObject.setObjectProp(controlName+"___userinput","title",null,ContainsAmp(Selectedstr));
    ircaObject.setObjectValue(controlName + "___userinput", rw.getCell(1).getValue());
    ircaObject.setObjectProp(controlName + "___userinput", "title", null, Selectedstr);
    ircaObject.setObjectValue(controlName + "___Value", rw.getCell(0).getValue());
    if (onSelectPopupValue) { onSelectPopupValue(); }
}
...
function setSuggestMultiValue(arrRows) {
    ...
    //ircaObject.getObject(controlName+'___selectionDiv').innerHTML="";
    clearSuggestGrid(controlName);
    resetArray(vids);
    AssDefaultValue(controlName + "___suggestholder", ContainsAmp(Selectedstr), 2);
    ircaObject.setObjectValue(controlName + "___Value", vids);
}
...
function XPAlert(obj, title, Msg, delay, iconX) {
    ...
    if (!JSBObj) { JSBObj = new JSBalloon(); };
    if (delay == -1 || delay == 0) {
...
function Add2Lstbx(cntrl, sText, SValue, PDOC, StrOptionArgumentList) {
    ...
    try {
        ...
        if (!myRcat || !StrOptionArgumentList) { return true };
...
function hr_onkeypress(hrcntrl, ev) {
    ...
    if (cp == 2 && parseInt(hrcntrl.value, 10) > 1 && charact > 51) { (ev.keyCode) ? ev.keyCode = 0 : charact = 0; return false; }
    if (cp == 2 && parseInt(hrcntrl.value, 10) > 2) { (ev.keyCode) ? ev.keyCode = 0 : charact = 0; return false; }
    if (cp == 1 && parseInt(hrcntrl.value, 10) < 4 && ev.keyCode < 51) { return true; }
    if (cp == 1 && parseInt(hrcntrl.value, 10) > 1 && ev.keyCode > 49) { (ev.keyCode) ? ev.keyCode = 0 : charact = 0; return false; }

}

function mi_onkeypress(mncntrl, ev) {
    ...
    if (cp == 2 && parseInt(mncntrl.value, 10) > 4 && ev.keyCode > 57) { (ev.keyCode) ? ev.keyCode = 0 : charact = 0; return false; }
    if (cp == 2 && parseInt(mncntrl.value, 10) > 5) { (ev.keyCode) ? ev.keyCode = 0 : charact = 0; return false; }
    if (cp == 1 && ev.keyCode < 54) { return true; }
    if (cp == 1 && parseInt(mncntrl.value, 10) > 5 && ev.keyCode > 53) { (ev.keyCode) ? ev.keyCode = 0 : charact = 0; return false; }

}

function hr_onkeydown(hrcntrl, ev) {
...
function mi_onkeydown(mncntrl, ev) {
...
function TimeControl(divn) {
    ...
    var dvnm = " <INPUT readonly language='javascript' onpaste='return false;' onbeforepaste='return false;' onkeypress='return hr_onkeypress(this,event);' IRCAType id='" + divn + "_hr' onkeydown='return hr_onkeydown(this,event);'" +
				...
				" onfocus='return hr_onfocus(this);' type='text' maxLength='2' size='1' name='hr' value='00'>:<INPUT readonly language='javascript' onkeypress='return mi_onkeypress(this,event);' IRCAType id='" + divn + "_mi' onkeydown='return mi_onkeydown(this,event);'" +
				" style='WIDTH: 16px; BORDER-TOP-STYLE: none; BORDER-RIGHT-STYLE: none; BORDER-LEFT-STYLE: none; HEIGHT: 16px; BACKGROUND-COLOR: transparent; TEXT-ALIGN: right; BORDER-BOTTOM-STYLE: none;'" +
...
function HashTable() {
    ...
    function _add() {
        if (arguments.length == 2 && arguments[1] == ',') {
            var itms = arguments[0].split(','); for (var i in itms) add(itms[i], '');
...
function CreateslideMenu(id) {
    ...
    sobj.expandcollapse = function(Grpid, boolexpand) {
        ...
        for (var i = 0; i < divHldrs.length; i++) {
            ...
            if (child.parentElement.id == Grpid) {
                child.style.display = (boolexpand == true ? '' : 'none');
    ...
    if (document.readyState == "complete") {
        ...
        if (isIEX) {
            ...
            addEventListener('onreadystatechange', init_slidemenu, false);
    }
    return this.obj;
};


//------------------------End of CreateslideMenu----------------------------
