//"ALL RIGHTS RESERVED (R) 2012 Onwards. CopyRight IRCA (INDIA) Pvt. Ltd."//
...
function getsText(strHTML) {
    ...
    //    var onerrorfuncRef;
    //    if (window.onerror) {
    //        onerrorfuncRef = window.onerror;
    //        window.onerror = function() { return true };
    //    }
    try {
        var tmpDivObj = document.createElement('DIV');

        if (tmpDivObj.innerHTML) {
            ...
            var tmpTxtNde = document.createTextNode(strHTML.replace('&sect', '& sect'));

            tmpTxtNde.textContent.replace('& sect', '&sect');

            tmpDivObj.appendChild(tmpTxtNde);

            return tmpDivObj.textContent;
        }
    } catch (r) {
        alert(r); return '';
    } finally {
...
function isTextHTML(strText) {
    ...
    try {
        //alert(strText.length + ' - ' + getsText(strText).length);
...
//------------------------End of String additional methods------------------
        ...
        function processItemres(cntrlname, cntrlpgname, Itemxml) {
            ...
            switch (cntrltype) {
                case "tree":
                    ...
                    if (PDOC.parentWindow.onCancel) {
                        PDOC.parentWindow.onCancel();
                        if (PDOC.parentWindow.addMode) { PDOC.parentWindow.addMode = false; }
                    }
                    break;
...
function IRCAOBJ(windref, IRCAMAINOBJref, IsMain) { // constructor
    this.window = windref || window;
    this.document = document;
    this.IRCAMAIN = IRCAMAINOBJref;
    this.page = '';
    this.id = "IRCAGEN_" + document.title;
    this.name = (name != undefined) ? name : document.title;
    try { this.pname = opener.name || pname }
    catch (e) { };
    this.pname += '';
    this.query = (qrystr != undefined) ? qrystr : '';
    this.isWindowRefresh = false;
    this.dtformat = "";
    this.PageInitB = false;
    this.pageTitle = '';
    this.DelString = '';
    this.isPageCanceled = false;
    this.enablePartialLoadCaching = true;
    this.plCache = [];
    this.Window_Init(IsMain);
    this.dhtmlwindow = (dhtmlwindow != undefined) ? dhtmlwindow : '';
    this.disableDataCall = null;
    this.parentWindow = (this.window) ? this.window.parent : null;
};

//IRCAOBJ.prototype.ircaMain = function() {
//    return this.IRCAMAIN;
//};

IRCAOBJ.prototype.Window_Init = function(IsMain) {
    if (this.ircaMain()) {
        this.ircaMain().unLockPg = ""
    };
    if (!this.document.oncontextmenu) {
        //this.document.oncontextmenu = function(ev) { this.DocContext(ev); }
    };
    this.document.onselectstart = function(ev) { /*this.DocContext(ev);*/ }
    if (this.document.onkeydown == null) {
        //this.document.onkeydown = function(ev) { this.DocKD(ev); }
    };
    this.docready = function() {
        var Objirca = ircaObject;
        Objirca.Page_State(Objirca);
    };
...
IRCAOBJ.prototype.Page_InitSplForNetAdv = function(IsMain) {
    if (!IsMain) {
        if (!this.igtbl_initGrid) {
            this.initGrid = [];
            this.igtbl_initGrid = function(gridId) {
                this.initGrid[this.initGrid.length] = gridId;
                this.ircaObject.includeGridScripts();
            }
        }
        if (!this.igtree_initTree) {
            this.initTree = [];
            this.igtree_initTree = function(treeId) {
                this.initTree[this.initTree.length] = treeId;
                this.ircaObject.includeTreeScripts();
            }
        }
        if (!this.ig_CreateWebScheduleInfo) {
            this.initWebScheduleFuncs = [];
            var oWebScheduleInfo1Obj = {
                window: this.window,
                updateControlState: function() {
                    this.initWebScheduleFuncs[this.initWebScheduleFuncs.length] = ['oWebScheduleInfo1.updateControlState', arguments];
                }
            };
            this.ig_CreateWebScheduleInfo = function() {
                this.initWebScheduleFuncs[this.initWebScheduleFuncs.length] = ['ig_CreateWebScheduleInfo', arguments];
                return oWebScheduleInfo1Obj;
            }
            this.ig_CreateCalendar = function() {
                this.initWebScheduleFuncs[this.initWebScheduleFuncs.length] = ['ig_CreateCalendar', arguments];
            }
            this.ig_CreateWebDialogActivator = function() {
                this.initWebScheduleFuncs[this.initWebScheduleFuncs.length] = ['ig_CreateWebDialogActivator', arguments];
            }
            this.igcal_init = function() {
                this.initWebScheduleFuncs[this.initWebScheduleFuncs.length] = ['igcal_init', arguments];
            }
            this.ig_CreateWebMonthView = function() {
                this.initWebScheduleFuncs[this.initWebScheduleFuncs.length] = ['ig_CreateWebMonthView', arguments];
            }
        }
    }
    this.igtbl_tableMouseMove = this.dumpF;
    this.igtbl_headerMouseOut = this.dumpF;
    this.igtbl_headerMouseMove = this.dumpF;
    this.igtbl_headerMouseOver = this.dumpF;
    this.igtbl_cellMouseMove = this.dumpF;
    this.igtbl_cellMouseOver = this.dumpF;
    this.igtbl_cellMouseOut = this.dumpF;
    this.igtree_mouseover = this.dumpF;
    this.igtree_mouseout = this.dumpF;
    this.igtree_onscroll = this.dumpF;
    if (!this.igtbl_activate) this.igtbl_activate = this.dumpF;
};
...
IRCAOBJ.prototype.Page_Init = function() {
    ...
    if (!this.allowSumbit) {
        this.document.forms[0].attachEvent("onsubmit", function() {
            return this.allowSumbit == true
        });
    }
    //if(this.initGrid.length>=1){this.Grid_Init();}
    //if(this.initTree.length>=1){this.Tree_Init();}
    if (this.initWebScheduleFuncs && this.initWebScheduleFuncs.length > 0) {
        this.WebSchedule_Init();
    }
    ...
    if (!this.disableDataCall)
        setTimeout(new initDataCall(this, null, true, true), 1);
    return;
};
...
IRCAOBJ.prototype.clearPartialCache = function(eventTarget, eventArgument) {
    var uniqueID = this.getHash(eventTarget + eventArgument);
    this.plCache[uniqueID] = null;
};
IRCAOBJ.prototype.XPInfo = function(ObjId, infoText, title) {
    if (!this.XPAlert || !ObjId) { return };
    if (!title) { title = this.pageTitle };
...
IRCAOBJ.prototype.ShowParentPopup = function(Url, title, type, isModal, windw) {
    ...
    if (typeof (type) == 'undefined' || type == null) type = 'iframe';
    if (!windw) { windw = this.window; /*if(!windw.isPopup)windw=this.ircaMain().ChildWB*/ };
...
IRCAOBJ.prototype.ShowPopupDiv = function(divID, title, prop, isModal, windw) {
    ...
    if (!this.dhtmlwindow) return null;
    if (!windw) windw = this.window;
    if (prop) { prop = prop + ",resize=1,scrolling=1,center=1"; }
    else { prop = "resize=1,scrolling=1,center=1" }
    var tempWind = this.dhtmlwindow.open(divID, "div", divID, title ? title : '', prop, windw, false, false, false, isModal);
    return tempWind;
};
IRCAOBJ.prototype.getHash = function(forString) {
    return this.ircaMain()._md5HashAlg.hex_md5(forString);
};
IRCAOBJ.prototype.getUniqueID = function() {
    return this.ircaMain()._md5HashAlg.hex_md5(new Date().toString());
};
IRCAOBJ.prototype.ircaMain = function() {
    return this.IRCAMAIN || this.IRCAMAIN
};
IRCAOBJ.prototype.splitStrA = "|~|";
IRCAOBJ.prototype.splitStrB = "|^|";
IRCAOBJ.prototype.escapeChars = function(strText) {
    strText += '';
    var _strText = strText.replace("'", "\'");
    _strText = _strText.replace('"', '\"');
    _strText = _strText.replace('\n', '').replace('\r', ''); /*_strText=_strText.replace(jsCRLF,'\\r');*/
    return _strText
};
IRCAOBJ.prototype.dumpF = function() { }
IRCAOBJ.prototype.getWindow = function() { return this.window }
IRCAOBJ.prototype.isNothing = function(objchk) {
    return this.document.getElementById(objchk) == null ? true : false;
}
IRCAOBJ.prototype.isWindAlive = function(objchk) {
    if (!objchk) { return false };
    var windw = null;
    if (typeof (objchk) == 'string') { windw = this.ircaMain().getWindowByName(objchk) }
    else { windw = objchk };
    if (typeof (windw) == 'string') { return false };
    if (!windw.closed && windw.location) { return true }; return false;
}
IRCAOBJ.prototype.isEmpty = function(objchk, propchk) {
    if (!propchk) { propchk = 'value' };
    return this.getObjectProp(objchk, propchk) == '' ? true : false;
}
IRCAOBJ.prototype.getObject = function(objchk) {
    return this.document.getElementById(objchk);
}
IRCAOBJ.prototype.getObjectValue = function(obj) {
    return this.document.getElementById(obj) == null ? "" : this.getObject(obj).value.trim();
}
IRCAOBJ.prototype.getObjectProp = function(obj, propstr) {
    if (!this.getObject(obj)) { return "" };
    return this.eval("document.getElementById('" + obj + "')." + propstr);
}
IRCAOBJ.prototype.setObjectValue = function(obj, xvalue, typ) {
    if (!typ) { typ = "0" };
    var shtml = false; var typ = typ.split("/");
    if (typ.length > 1 && typ[1].toLowerCase() == "h") { shtml = true };
    if (!this.getObject(obj) || xvalue == null) { return };
    xvalue = this.escapeChars(xvalue);
    var tmpobj = this.getObject(obj);
    switch (parseInt(typ[0], 10)) {
        case 0: tmpobj.value = xvalue; break;
        case 1:
            if (shtml) { tmpobj.innerHTML = xvalue }
            else { tmpobj.innerText = xvalue };
            break;
        case 2:
            var sval = ""; var stext = "";
            if (typeof (xvalue == "string")) { sval = xvalue; stext = sval }
            else { sval = xvalue[0][0]; stext = xvalue[0][1] }
            Add2Lstbx(tmpobj, stext, sval, this.document);
            break;
        case 3: tmpobj.checked = xvalue; break;
    }
}
IRCAOBJ.prototype.getObjectSrc = function(obj) {
    try { return this.getObject(obj).src }
    catch (e) { return '' }
}
IRCAOBJ.prototype.setObjectSrc = function(obj, xvalue) {
    try { this.getObject(obj).src = xvalue }
    catch (e) { }
}
IRCAOBJ.prototype.setObjectProp = function(obj, propstr1, propstr2, pvalu, dquotepv) {
    if (!this.getObject(obj)) { return false };
    if (!propstr1) { return false };
    if (pvalu == null) { return false };
    var propstr;
    if ((typeof (pvalu) == 'boolean') || (typeof (pvalu) == 'number')) { dquotepv = true };
    if (!dquotepv && dquotepv != 0) { pvalu = "'" + pvalu.replace("'", "\\'") + "'" };
    if (propstr2) { propstr = propstr1 + "." + propstr2 }
    else { propstr = propstr1 };
    var scpt = "document.getElementById('" + obj + "')." + propstr + "=";
    if (typeof (pvalu) == 'boolean') { scpt += pvalu }
    else { scpt += pvalu == "" ? "''" : pvalu };
    if (scpt == "") { return false };
    try { this.execScript(this.escapeChars(scpt)); }
    catch (e) { };
    return true
}
IRCAOBJ.prototype.fireObjectMethod = function(obj, method, param) {
    if (!param) { param = '' };
    try { this.eval("document.getElementById('" + obj + "')." + method + '(' + param + ');') }
    catch (e) { }
}
IRCAOBJ.prototype.copyObjectValue = function(fromstr, tostr) {
    try { this.getObject(tostr).value = this.getObjectValue(fromstr) }
    catch (ex) { return false }
    return true
};
IRCAOBJ.prototype.pause = function(timeout) {
    var dialogScript = 'setTimeout(' + ' function () { close(); }, ' + timeout + ');';
    var disablwin = this.showModalDialog('javascript:this.document.writeln(' + '"<script>' + dialogScript + '<' + '/script>")');
}
IRCAOBJ.prototype.queryString = function(key) {
    var page = new PageQuery(this.location.search); return unescape(page.getValue(key));
}
IRCAOBJ.prototype.toolTipHelp = function(vartxt, ev) {
    try {
        var oPopup = this.createPopup();
        var oPopBody = oPopup.document.body;
        oPopBody.style.backgroundColor = "lightyellow";
        oPopBody.style.border = "solid black 1px";
        oPopBody.innerHTML = vartxt;
        oPopup.show(ev.clientX, ev.clientY, 180, 125, document.body);
    }
    catch (e) { }
}
IRCAOBJ.prototype.repeatString = function(strInput, intCount) {
    var arrTmp = new Array(intCount + 1);
    return arrTmp.join(strInput);
}
IRCAOBJ.prototype.checkEmail = function(e) {
    re = /(@.*@)|(\.\.)|(^\.)|(^@)|(@$)|(\.$)|(@\.)/;
    re_two = /^.+\@(\[?)[a-zA-Z0-9\-\.]+\.([a-zA-Z]{2,3}|[0-9]{1,3})(\]?)$/;
    if (!e.match(re) && e.match(re_two)) { return true; }
    else { return false };
}
IRCAOBJ.prototype.CWBNavigate = function(CWBpgstr, hndlcrossref) {
    CWBNavigate(CWBpgstr, hndlcrossref);
}
...
IRCAOBJ.prototype.validateObjectValues = function(arrobjstr) {
    var cont = 0;
    if (arrobjstr.length < 1) { return false };
    for (var x = 0; x < arrobjstr.length; x++) {
        if (!this.getObjectValue(arrobjstr[x])) { cont++ }
    };
    return cont == (arrobjstr.length) ? true : false
}
// if was found:this in DocKD refers to Document instead of IRCAOBJ--so thisfunction is modified diff.
IRCAOBJ.prototype.DocKD = function(ev) {
    try {
        var thishkey = (ev.keyCode) ? ev.keyCode : ((ev.charCode) ? ev.charCode : ev.which);
        var Element = ev.srcElement ? ev.srcElement : ev.target;
        switch (thishkey) {
            case 114: case 18: case 116: case 115: case 122: case 114: case 117: case 112: case 118:
                try {
                    ev.cancelBubble = true; ev.returnValue = false; ev.keyCode = 0;
                }
                catch (e) {
            ...
            case 66: case 69: case 70: case 73: case 72: case 76: case 79: case 78: case 80: case 82: case 87: case 119:
                ...
                if (this.parentWindow.parent.handleeventkb) { this.parentWindow.parent.handleeventkb(thishkey) }; break;
            case 13:
                //ev.keyCode = 0;ev.cancelBubble = true;ev.returnValue = false;
                if (this.parentWindow.handleeventkb) { this.parentWindow.handleeventkb(thishkey) }; /*break;*/
                if (!ev.shiftKey && Element.type != "text" && Element.tagName != "INPUT") { return }
        }
    }
    catch (e) { }
};
IRCAOBJ.prototype.DocContext = function(ev) {
    try {
        //return true;
        var Element = ev.srcElement ? ev.srcElement : ev.target;
...
IRCAOBJ.prototype.ThisPage = function(thisdoc) {
    ...
    if (strpg == "") {
        var strpg = thisdoc.URLUnencoded + "";
        var splstr = strpg.split("/");
        if (splstr.length > 0) { strpg = splstr[splstr.length - 1]; }
    }
    return strpg;
    //return this.ircaMain().ThisPage(thisdoc);
};
...
//IRCAMAINOBJ.appConfigManager = IRCAMAINOBJ.getNewdomXml('appConfigManager', true);
//IRCAMAINOBJ._md5HashAlg = IRCAMAINOBJ.md5HashAlg();
...
var IRCAMAIN = new IRCAMAINOBJ();
IRCAMAIN.IRCAWind = (name) ? name : '';
var IRCA = new IRCAOBJ(null, null, true); //tell IRCAOBJ that this is main
var ircaObject = IRCA;
ircaObject.IRCAMAIN = IRCAMAIN;
