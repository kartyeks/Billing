<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Master_Maritial_Status.aspx.cs" Inherits="AltioStarHR.Masters.Master_Maritial_Status" %>
...
<div style="position:relative;width:100%">
   ...
   <strong class="family1 imageheadercolor">Marital Status</strong>
    </div>
</div>
    <div class="dvGrid" id="divmartialstatus" style="display:none;width:100%;padding-left:1%;background-color:White">
        <form id="signupForm" method="get" action="">
        <fieldset style="width:97%;margin-left:1%;margin-top:0%;background-color:#a4cd0f">
            <table style="width:100%">
                  <tr>
                        <td style="width:20%">
                            <label id="lmaritalstatus" for="maritals" class="family">
                                Marital Status</label>
                            <span style="font-size: medium; color: Red">*</span>
                        </td>
                        <td style="width:18%">
                            <input id="maritals" name="maritals" type="text" tabindex="1" value="" style="width: 220px;border:1px solid silver"
                            class="family" />
                        </td>
                        <td>
                        </td>
...
//        }).trigger('resize');
    ...
    jQuery(document).ready(function() {
        ...
        var Response = SendApplicationRequest('<%=HRAppCommands.GET_ALL_MARITALSTATUS_DETAILS%>', '', true);

        Response.ResponseObject.colModel = [
            { name: 'id', index: 'id', width: 20, hidden: true, sorttype: "int" },
            { name: 'martialstatus', index: 'martialstatus', width: 100, editable: true },
...
//        Response.ResponseObject.caption = "MaritalStatus";
        ...
        $("button,input", "#t_list").click(function(id) {
            ...
            if (id.currentTarget.id == "add") {
                ...
                document.getElementById("divmartialstatus").style.display = "";
            }
...
//        $(cDiv).hide();
        ...
        var validator = $("#signupForm").validate({
            ...
            messages: {
            maritals: {
            alphaNumerix: "Invalid Marital Status Name",
            required: "Enter Marital Status Name",
            ...
            submitHandler: function(id) {
                ...
                Request.ID = ID;
                Request.MaritalStatus = $.trim($("#maritals").val());
                ...
                //Request.UpdateBy=1;
                var Response = SendApplicationRequest('<%=HRAppCommands.UPDATE_MARITALSTATUS%>', Request, true);

                alert(Response.Message);
                if (Response.ResponseCommand == 'SUCCESS') {
                    mode = "SUCCESS";
                    var Response = SendApplicationRequest('<%=HRAppCommands.GET_ALL_MARITALSTATUS_DETAILS%>', Request, true);
   ...
   function ActivateRow(ROWID) { 
    var ret = jQuery("#list").jqGrid('getRowData', ROWID);
        ...
        var Response = SendApplicationRequest("<%=HRAppCommands.GET_ACTIVE_DEACTIVATE_MARITALSTATUS%>", Request, true);
        alert(Response.Message);
...
function onCancel() {
    ...
    document.getElementById("divmartialstatus").style.display = "none";
}
function fillData() {

    var id = jQuery("#list").jqGrid('getGridParam', 'selrow');
    var ret = jQuery("#list").jqGrid('getRowData', id);
    $("#maritals").val(ret.martialstatus);
    
    
    if (ret.activestatus == 'False') { $("#isactive").attr('checked', false); }
