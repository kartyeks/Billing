<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="InvestmentGroup.aspx.cs" Inherits="AltioStarHR.Masters.InvestmentGroup" %>

<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head id="Head1" runat="server">
    <title>InvestmentGroup</title>
...
<div style="position:relative;width:100%">
   ...
   <strong class="family1 imageheadercolor">InvestmentGroup</strong>
    </div>
</div>
    <div id="divGroupName" class="dialog" style="display: none;background-color:White;width:100%">
                ...
                <tbody>
                    <tr>
                        <td class="label" width="200" height="40" >
                            <label id="lInvestmentgroupName" for="GroupName" class="family">
                                Investment Group Name</label>
                                <span style="font-size: medium; color: Red">*</span>
                        </td>
                        <td class="field" width="200" height="40">
                            <input id="GroupName" name="GroupName" style="width: 220px;border:1px solid silver" type="text" value=""
...
////    }).trigger('resize');
    ...
    jQuery(document).ready(function() {
        ...
        var myGrid = $('#list');
        var Response = SendApplicationRequest('<%=HRAppCommands.GET_ALL_INVESTMENTGROUP_DETAILS%>', '');
        Response.ResponseObject.colModel = [
            { name: 'ID', index: 'ID', width: 20, hidden: true, sorttype: "int" },
            { name: 'InvestmentGroupName', index: 'InvestmentGroupName', width: 100, editable: true,sortable:false },
...
//        jQuery("#list").jqGrid('setGridParam', { page: 1 }).trigger("reloadGrid");
        ...
        $("button ,input", "#t_list").click(function(id) {
            if (id.currentTarget.id == "add") {
                ...
                document.getElementById("divGroupName").style.display = "";
...
//        $(cDiv).hide();
       // $("#list").closest(".ui-jqgrid-bdiv").height("auto");
        ...
        var validator = $("#signupForm").validate({
            ...
            messages: {
                RelationName: {
                    required: "Enter InvestmentGroup Name",
                    alphaNumerix: "Invalid InvestmentGroup Name"
                }
            },
            ...
            submitHandler: function(id) {
                ...
                Request.GroupName = jQuery.trim($("#GroupName").val());
                if ($("#isactive").is(':checked')) { Request.IsActive = true; }
                else { Request.IsActive = false; }

                var Response = SendApplicationRequest('<%=HRAppCommands.UPDATE_INVESTMENTGROUP%>', Request, true);
               
                alert(Response.Message);
                if (Response.ResponseCommand == 'SUCCESS') {
                    mode = 'SUCCESS';
                      var Response = SendApplicationRequest('<%=HRAppCommands.GET_ALL_INVESTMENTGROUP_DETAILS%>', Request, true);
       ...
       function ActivateRow(ROWID) { 
        ...
        var Response = SendApplicationRequest("<%=HRAppCommands.ACTIVATE_INVESTMENTGROUP%>", Request, true);
        alert(Response.Message);
    ...
    function DeActivateRow(ROWID) {
        ...
        var Response = SendApplicationRequest("<%=HRAppCommands.DEACTIVATE_INVESTMENTGROUP%>", Request, true);
        alert(Response.Message);
