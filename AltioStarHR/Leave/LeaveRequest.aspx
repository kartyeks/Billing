<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="LeaveRequest.aspx.cs" Inherits="AltioStarHR.Leave.LeaveRequest" %>
...
<body>
   <%-- <div id="tabs" class="demo">--%>
    <div id="tabs" style="padding-left:2%;border:none">
        <%--<ul style="background-color:#02d9f5">--%>
        <ul style="background-color:none;border:none">
            <li id="lLeaveRequest"><a href="#tabs-1">
                <label class=" imageheadercolor" style="color:Black;font-family:Arial;font-size:12px;font-weight:bold">
                    Leave Overview</label></a></li>
            <li id="lLeaveEncashment"><a href="#tabs-2" >
                <label class="family1 imageheadercolor" style="color:Black;font-family:Arial;font-size:12px;font-weight:bold">
                    Leave Request</label></a></li>
        </ul>
        <div id="tabs-1">
            <div style="position:relative;width:100%">
                <img id="img1" alt="" src="Resources/Images/NewImages/1.png" style="width:100%;margin-left:-2%;height:53px;margin-top:-1%"/>
                <div style="position:absolute;left:0%;top:30%;margin-top:0.5%;">
                <strong class="family1 imageheadercolor">Leave Overview</strong>
                </div>
            </div>
            <div style="margin-left:-2%;" id="box" style="width:100%">
                <div id="LeaveOverviewGrid">
                </div>
            </div>
        </div>
        <div id="tabs-2">
                <div style="position:relative;width:100%;">
                    <img alt="" src="Resources/Images/NewImages/1.png" style="width:100%;margin-left:-2%;height:53px;margin-top:-1%"/>
                    <div style="position:absolute;left:0%;top:30%;margin-top:0.5%;">
                    <strong class="family1 imageheadercolor">Leave Request</strong>
                    </div>
                </div>
            <div style="margin-left:-2%;" id="box1">
                <div id="LeaveRequestGrid" style="width:100%">
                </div>
            </div>
        </div>
    </div>
    <div id="divLeaveRequest" title="Leave Request Details" style="display: none;"> 
        <form id="signupFormLeaveReq" method="get" action="" style="background-color:White">
                    ...
                    <table class="teble_bg" style="width: 360px">
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="statusOnlyStar" id="sfromdate" style="width: 150px; font-size: x-small;">
                                </td>
                            </tr>
                            <tr>
                                <td class="label">
                                ...
                                <td class="field">
                                    <input type="text" id="todate" name="todate" style="width: 80px;border:1px solid silver" readonly="readonly" onblur="FillLeaveDaysCount();" />
                                     <input type="checkbox" id="todatehalfday"name="halfday" value="true" onchange="FillLeaveToDateHalfDaysCount();">Halfday<br>
                                </td>
                                <td class="statusOnlyStar" id="stodate" style="width: 150px; font-size: x-small;">
                                </td>
                            </tr>
                            <tr>
                                <td class="label">
                                ...
                                <td class="field">
                                    <input type="text" id="daycount" readonly="readonly" name="daycount" style="width: 150px;border:1px solid silver" />
                                </td>
                            ...
                            <tr>
                                ...
                                <td class="statusOnlyStar" id="sreason" style="width: 150px; font-size: x-small;">
                                </td>
                            </tr>
                            <tr>
                                <td colspan="3">
                                    <input id="signupsubmit" name="Submit" tabindex="4" type="submit" value="Submit"
                                        style="border:1px solid silver;background-color:#00DBF1;" class="family" />
    ...
    <input type="hidden" runat="server" id="hdnLeaveCount" />
    <input type="hidden" runat="server" id="hdnFromLeaveCount" />
    <input type="hidden" runat="server" id="hdnToLeaveCount" />
</body>
...
<script type="text/javascript">
    $("#box").css({ "background-image": "url(Resources/Images/NewImages/dashboard_bg.gif)", "width": "96.5%" });
    $("#box1").css({ "background-image": "url(Resources/Images/NewImages/dashboard_bg.gif)", "width": "96.5%" });   
        ...
        validatorLeave = $("#signupFormLeaveReq").validate({
            ...
            submitHandler: function(id) {
                ...
                if (Response.ResponseCommand == 'SUCCESS') {
                    ...
                    var boxH = (document.documentElement.clientWidth * ($('#box').width() - 0.5)) / 100;
                    var boxH2 = (document.documentElement.clientWidth * ($('#box').width() - 2)) / 100;

                    $(".listpager").jqGrid('setGridWidth', boxH2);
                    $("#list2").jqGrid('setGridWidth', boxH);
                }

                $("#list").jqGrid('setGridParam', { page: currPage }).trigger('reloadGrid');
                $("#list2").jqGrid('setGridParam', { page: currPage }).trigger('reloadGrid');
    ...
    function LoadLeaveRequest() {
        ...
        $("#LeaveRequestGrid").html("<div><table class='listpager' id='list'></table><div class='listpager' id='pager'></div></div>");
        var myGrid = $('#list');
       ...
       // $("#list").closest(".ui-jqgrid-bdiv").height("auto");
        $("#t_list2").closest(".ui-userdata").hide();
        $("#box").css({ "background-image": "url(Resources/Images/NewImages/dashboard_bg.gif)", "width": "96.5%" });
        $("#box1").css({ "background-image": "url(Resources/Images/NewImages/dashboard_bg.gif)", "width": "96.5%" });   
        
        jQuery("#list").jqGrid('setGridParam', { page: 1 }).trigger("reloadGrid");
        $("#t_list").append("<button type='button' id='addrequest' value='Add' style='background:none;border:0px;outline:none;'><img src='Resources/Images/NewImages/add_icon.png' height=18px;width=18px alt='' /><br/><span>Add</span></button>");
        $("#t_list").append("<button type='button' id='editrequest' value='Modify' style='background:none;border:0px;outline:none;'><img src='Resources/Images/NewImages/leave_modify.png'height=18px;width=18px alt='' /><br/><span>Modify</span></button>");
        $("#t_list").append("<button type='button' id='deleterequest' value='Delete' style='background:none;border:0px;outline:none;'><img src='Resources/Images/NewImages/delete_intimation.png' height=18px;width=18px alt='' /><br/><span>Delete</span></button>");
        $("input,button", "#t_list").click(function(id) {
            if (id.currentTarget.id == "addrequest") {
                if ($("#hdnBalanceLeave").val() <= 0) {
                    alert("Your Balance Leave is Zero.");
                    return false;
                }
            ...
            else if (id.currentTarget.id == "deleterequest") {
                var id = jQuery("#list").jqGrid('getGridParam', 'selrow');
                if (id) {
                    var ret = jQuery("#list").jqGrid('getRowData', id);
                    ...
                    if (ret.status == "Requested") {                      
                        ...
                        Request.ID = ret.id;
                        Request.EmployeeID = $('#hdnempid').val();
                        if (confirm('Do you want to Delete?') == false) return false;
                        var Response = SendApplicationRequest('<%=HRAppCommands.DELETE_LEAVE_REQUEST%>', Request, true);
                        //to reload the grids
                        alert(Response.Message);
                        mode = "SUCCESS";
                        var Req = new Object();
                        Req.EmployeeID = $("#hdnempid").val();
                        LoadLeaveRequest();
                        var Response2 = SendApplicationRequest('<%=HRAppCommands.LEAVE_OVERVIEW_DETAILS%>', Req, true);
                        if (Response2.ResponseObject.datastr == null) {
                    ...
                    else if (ret.status == "Approved" || ret.status == "Reject") {
                        alert("Approved/Rejected request cannot be edited/deleted");
                        return null;
                    }
                    else if (ret.Status == "Approved") {
                        alert("Leave Is in Process Cannot Edit");
                        return null;
                    }
                    else {
                        alert("You can't delete approved or rejected leaves");
                    }
                } else {
...
//    }
    ...
    function FillLeaveDaysCount() {        
        ...
        if ($("#hdnRequestedLeave").val() < CountLeave && ($("#hdnBalanceLeave").val()< CountLeave)) {
            ...
            var leavecount = $("#daycount").val();   
            $("#hdnLeaveCount").val(leavecount); 
            if(CountLeave==1)
            {
    ...
    function LoadLeaveOverview() { 
        ...
        $("#LeaveOverviewGrid").html("<div ><table id='list2'></table></div>");
        var myGrid = $('#list2');
        ...
        if(Response.ResponseObject.datastr.rows[0].cell[8]!= 2 && $("#ltype").val()!= 1)
        {            
